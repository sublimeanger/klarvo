...
interface DigestRequest {
  ...
  forceRefresh?: boolean; // Skip frequency check
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { userId, forceRefresh }: DigestRequest = await req.json().catch(() => ({}));
    ...
    for (const user of users || []) {
      try {
        // Check frequency (skip if not forcing)
        if (!forceRefresh && user.last_notification_sent_at) {
          const lastSent = new Date(user.last_notification_sent_at);
          const now = new Date();
          const hoursSinceLast = (now.getTime() - lastSent.getTime()) / (1000 * 60 * 60);
          
          if (user.notification_frequency === "daily" && hoursSinceLast < 23) {
            continue;
          }
          if (user.notification_frequency === "weekly" && hoursSinceLast < 167) {
            continue;
          }
        }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p style="margin: 5px 0 0;">Your ${user.notification_frequency} compliance summary</p>
    </div>
    <div class="content">
      ...
      <div class="footer">
        ...
        await supabase.from("notification_logs").insert({
          ...
          details: { alertCount: alerts.length, criticalCount: criticalAlerts.length },
        });
    ...
    console.error("Error in send-compliance-digest:", error);
    return new Response(
