...
serve(async (req) => {
  ...
  try {
    ...
    if (orgId) {
      const planCheck = await checkPlanEntitlement(supabaseUrl, supabaseServiceKey, orgId, 'ai_intake');
    ...
    if (rlUser) {
      const rateLimit = await checkRateLimit(supabaseUrl, supabaseServiceKey, rlUser.id, "ai-system-intake", 20, 15);
